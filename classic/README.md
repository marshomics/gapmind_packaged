...
## Large-scale analysis (many genomes)

For many genomes (thousands to hundreds of thousands), `make analyze` (one genome
at a time) is the wrong tool. Batch mode takes a tab-separated manifest, splits the
genomes into batches, runs one task per batch (on this machine or as an SGE array
job), and merges the results. It's resumable and the batch size only affects speed,
not the calls. No cluster is required: everything except `batch-submit` runs on a
single machine.
...
Both routes start the same way: prepare batches from the manifest.

```bash
make batch-prepare MANIFEST=genomes.tsv   # validates files, sanitizes names, stages messy paths
```

### On one machine (no cluster)

`make batch-local` runs every batch right here, then you merge and summarize. `JOBS`
is how many batches run at once; each still uses `SGE_SLOTS` cores (default 8), so
keep `JOBS × SGE_SLOTS` near your core count. It's resumable: finished batches are
skipped, so just re-run after a failure.

```bash
make batch-local JOBS=4                       # or JOBS=1 for one batch at a time
# on a 16-core box, cap total load:  SGE_SLOTS=4 make batch-local JOBS=4
make batch-merge && make plots && make presence
```

For a single proteome, skip batch mode entirely:

```bash
make analyze FAA=my_proteins.faa NAME=myorg SET='aa carbon'
```

### On an SGE cluster

```bash
make batch-calibrate       # time one batch to size batch/mem/walltime
# set SGE_* and BATCH_SIZE in config.sh from what you learned, then:
make batch-submit          # array job + a merge job held until the array finishes
make batch-status          # progress; the merge runs automatically when the array ends
```
...
GapMind reports confidence, not a yes/no call, so collapsing it to 0/1 needs a
stated rule. `make presence` (also run automatically by the merge job) writes, per
set, a `genome x pathway` matrix using GapMind's own operational definition rather
than an invented cutoff.

For each pathway in each genome, GapMind scores every step on the best-scoring path
as high, medium, or low confidence (in `gapsummary.pl`, from percent identity,
coverage, and bit score relative to the best "reverse" hit to an unrelated curated
protein), and the `rule=all` row totals them as `nHi`/`nMed`/`nLo`. The threshold:

- **`probably` (default)** — present (1) iff `nLo == 0`: every step is at least
  medium confidence. This is exactly what GapMind calls "probably present," and
  what the GapMind resource reports as pathway presence.
- **`strict`** — present (1) iff `nMed == 0 and nLo == 0`: every step high
  confidence. Fewer false positives, more false negatives (misses pathways carried
  by diverged or hard-to-detect enzymes).

Set the mode with `PA_MODE` in `config.sh` (or `make presence PA_MODE=strict`).

Outputs land in `batch/merged/`:

- `<set>.presence.tsv` — genome in column 1, then one 0/1 column per pathway.
- `<set>.confidence.tsv` — the same shape but `2` (high) / `1` (probably) / `0`
  (absent). Keep this: it preserves the full call so you can re-threshold, weight,
  or apply your own rule without recomputing (`presence(probably)` is exactly
  `confidence >= 1`; `presence(strict)` is `confidence == 2`).
- `<set>.pathways.tsv` — pathwayId → description legend for the columns.

Why keep both, and the honest caveats: a single cutoff can't be perfect. A pathway
with `nLo == 0` but several medium steps is called present but is less certain than
an all-high call — the `2/1/0` matrix is where that nuance lives. GapMind also
curates "known gaps" (steps that are legitimately hard to detect in organisms known
to have the pathway); the default rule treats a low-confidence step as absent even
when it might be a known gap, so for lineages close to GapMind's curated set the
`*.sum.knownsim` output (amino-acid set) flags genomes where a called gap may be a
known one. For a de-novo screen of 340k genomes the `nLo == 0` rule is the
practical, citable default; the confidence matrix lets you revisit any of this.
...
this pipeline wraps rather than re-implements. Attribution of the wrapped software
and third-party tools is in [`../NOTICE`](../NOTICE). If you use this pipeline, cite
the GapMind papers (Price et al., mSystems 2020 and PLOS Genetics 2022), listed in
[`../CITATION.cff`](../CITATION.cff). These files sit at the repository root and
cover both this pipeline and the Nextflow one.
